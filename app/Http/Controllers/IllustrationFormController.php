...
use App\Models\IllustrationFormData;

class IllustrationFormController extends Controller
{
    public function viewForm() {
        return view('ill-request-form');
    }
    ...
    public function storeRequest(Request $request) : RedirectResponse {
        ...
        $formData = new IllustrationFormData();
        $formData->journal_cover = $request->input('journal_cover');
        $formData->description = $request->input('description');
        $formData->deadline = $request->input('deadline');
        ...
        $formData->user_name = $request->input('user_name');
